...
describe('Contacts API Integration Tests', () => {
  ...
  it('should return 409 for duplicate email', async () => {
    ...
    const response = await app.inject({ method: 'POST', url: '/v1/contacts', payload });
  ...
  it('should filter contacts by search query', async () => {
    ...
    await app.inject({
      method: 'POST',
      url: '/v1/contacts',
      payload: { name: uniqueName, email: `${uniqueName}@example.com`, phone: '+55 11 98888-8888' }
    });
    ...
    expect(response.statusCode).toBe(200);
    const body = JSON.parse(response.payload);
    expect(body.data.some((c: any) => c.name === uniqueName)).toBe(true);
  });
  ...
  it('should fetch a single contact by ID', async () => {
    const createRes = await app.inject({
      method: 'POST',
      url: '/v1/contacts',
      payload: { name: 'Single', email: `single-${Date.now()}@example.com`, phone: '+55 11 98888-8888' }
    });
    const created = JSON.parse(createRes.payload);
  ...
  it('should allow partial updates', async () => {
    const createRes = await app.inject({
      method: 'POST',
      url: '/v1/contacts',
      payload: { name: 'Before', email: `update-${Date.now()}@example.com`, phone: '+55 11 98888-8888' }
    });
    const created = JSON.parse(createRes.payload);
  ...
  it('should delete a contact', async () => {
    const createRes = await app.inject({
      method: 'POST',
      url: '/v1/contacts',
      payload: { name: 'To Delete', email: `delete-${Date.now()}@example.com`, phone: '+55 11 98888-8888' }
    });
    const created = JSON.parse(createRes.payload);
