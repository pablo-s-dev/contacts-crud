...
export function createServer() {
  ...
  app.addHook("onSend", async (request: FastifyRequest, reply, payload) => {
    ...
    const startTime = (request as FastifyRequest & { startTime?: number })
      .startTime;
    const route =
      (request as FastifyRequest & { route?: string }).route || request.url;
    const method =
      (request as FastifyRequest & { metricsMethod?: string }).metricsMethod ||
      request.method;
    const metricsRoute =
      (request as FastifyRequest & { metricsRoute?: string }).metricsRoute ||
      route;
    const metricsStartTime = (
      request as FastifyRequest & { metricsStartTime?: number }
    ).metricsStartTime;
    ...
    if (metricsStartTime) {
      ...
      httpRequestDuration.observe(
        { method, route: metricsRoute, status_code: statusCode },
        duration,
      );
  ...
  app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof ZodError) {
      logger.warn(
        { correlationId, error: fromZodError(error).toString() },
        "Validation error",
      );
    ...
    logger.error(
      { correlationId, error: errorMessage, stack: errorStack },
      "Unhandled error",
    );
