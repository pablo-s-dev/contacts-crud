...
export async function createContact(app: FastifyInstance) {
  app.post('/contacts', {
  ...
  }, async (request: FastifyRequest<{ Body: z.infer<typeof CreateContactSchema> }>, reply: FastifyReply) => {
    const correlationId = getCorrelationId(request);
    const { name, email, phone } = request.body;

    // Check idempotency
    const idempotencyCheck = await checkIdempotency(request, 'create-contact');
    ...
    try {
      ...
      dbQueryDuration.observe({ operation: 'create', model: 'Contact' }, duration);
      ...
      logger.info({ correlationId, contactId: contact.id }, 'Contact created');
