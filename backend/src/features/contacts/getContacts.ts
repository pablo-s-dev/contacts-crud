...
import { decodeCursor, encodeCursor, buildCursorCondition } from '../../utils/cursor';
...
function getContactsSortSql(sortField: ContactsSortField, sortOrder: ContactsSortOrder) {
  const sortColumnSql = sortField === 'createdAt'
    ? Prisma.sql`"createdAt"`
    : sortField === 'email'
      ? Prisma.sql`email`
      : Prisma.sql`name`;
  const sortDirectionSql = sortOrder === 'desc' ? Prisma.sql`DESC` : Prisma.sql`ASC`;
  return { sortColumnSql, sortDirectionSql };
}
...
async function fetchContactsOffsetPhoneSearch(params: {
  ...
  const { sortColumnSql, sortDirectionSql } = getContactsSortSql(params.sortField, params.sortOrder);

  const qLike = `%${params.q}%`;
  const phoneLike = `%${params.normalizedPhoneQuery}%`;

  const contacts = await prisma.$queryRaw<any[]>(Prisma.sql`
  ...
  const totalResult = await prisma.$queryRaw<[{ count: number }]>(Prisma.sql`
...
export async function getContacts(app: FastifyInstance) {
  app.get('/contacts', {
  ...
  }, async (request: FastifyRequest<{ Querystring: z.infer<typeof GetContactsQuerySchema> }>, reply: FastifyReply) => {
    ...
    // Build cache key
    const cacheKey = buildContactsCacheKey({ q, page, pageSize, sort, order, cursor, pagination });
    ...
    try {
      ...
      if (pagination === 'keyset' && cursor) {
        ...
        try {
          ...
          logger.warn({ correlationId, cursor, error }, 'Invalid cursor format for keyset pagination');
          return reply.status(400).send({ 
            message: 'Invalid cursor format. Keyset pagination requires a properly encoded cursor.' 
          });
        }

        // Build cursor condition using utility function
        const cursorCondition = buildCursorCondition(sortField, cursorValue, cursorId, sortOrder);

        // Merge with existing where clause
        const cursorWhere = Object.keys(where).length > 0
        ...
        if (hasMore) {
          ...
          const sortValue = lastContact[sortField as keyof typeof lastContact];
      ...
      dbQueryDuration.observe({ operation: 'findMany', model: 'Contact' }, duration);

      const response = pagination === 'keyset' 
        ? {
            data: contacts,
            cursor: nextCursor,
            hasMore,
